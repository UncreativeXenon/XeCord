...
const std::string g_ValidPlatforms[] = {
    "xbox",
    "ps4",
    "ps5"
};

const size_t g_ValidPlatformCount = sizeof(g_ValidPlatforms) / sizeof(g_ValidPlatforms[0]);
...
const char* GetSafeGamertag() {
    if (XUserGetSigninState(0) != eXUserSigninState_NotSignedIn) {
        ...
        if (XUserGetName(0, gamertag, 16) == ERROR_SUCCESS) return gamertag;
    }
...
bool FetchDiscordAsset(uint32_t titleId, uint32_t secId, char* outAssetId, size_t outSize)
{
    bool success = false;
    ...
    XexUtils::Socket* apiSocket = new XexUtils::Socket("discord.com", 443, true);

    HRESULT hr = apiSocket->AddECTrustAnchor(EC_DN, sizeof(EC_DN), EC_Q, sizeof(EC_Q), XexUtils::Socket::Curve_secp384r1);
	...
	if (secId != 0) sprintf_s(iconUrl, 256, "https://raw.githubusercontent.com/UncreativeXenon/XboxUnity-Scraper/refs/heads/master/Icons/%08X.png", secId);
	else if (titleId == 0x5841149E) sprintf_s(iconUrl, 256, "https://raw.githubusercontent.com/UncreativeXenon/XboxUnity-Scraper/refs/heads/master/Icons/%s.png", "5841149E_1");
	else sprintf_s(iconUrl, 256, "https://raw.githubusercontent.com/UncreativeXenon/XboxUnity-Scraper/refs/heads/master/Icons/%08X.png", titleId);

    char* jsonBody = tempBuffer + 256;
    sprintf_s(jsonBody, 512, "{\"urls\":[\"%s\"]}", iconUrl);

	const char* super_props = "";

    char* request = new char[2048];
    sprintf_s(request, 2048,
        "POST /api/v9/applications/1410522131762253927/external-assets HTTP/1.1\r\n"
        "Host: discord.com\r\n"
		"Accept: */*\r\n"
		"Accept-Language: en-US,en;q=0.9\r\n"
        "Authorization: %s\r\n"
    ...
    if (apiSocket->Send(request, strlen(request)) > 0)
    {
        ...
        while (totalBytes < 8192 - 1) {
            int r = apiSocket->Receive(buffer + totalBytes, 8192 - totalBytes - 1);
            if (r <= 0) break; 
            totalBytes += r;
        }
        buffer[totalBytes] = '\0';
        ...
        if (pathKey)
        {
            ...
            while (valStart[i] != '"' && valStart[i] != '\0' && outIndex < outSize - 1)
            {
                if (valStart[i] != '\\') {
            ...
            // XexUtils::Log::Print("[XeCord] Constructed Asset: %s.", outAssetId);
            success = true;
        }
        else {
            XexUtils::Log::Print("[XeCord] Error: 'external_asset_path' not found in response.");
        }
...
bool SendPresenceUpdate(DiscordState* state, uint32_t titleId)
{
	...
	if (titleId == 0x00000000) {
		if (EndsWith(ExLoadedImageName, "Aurora.xex")) finalTitleId = 0x00000166;
		else if (EndsWith(ExLoadedImageName, "dash.xex")) finalTitleId = 0xFFFE07D1;
		else finalTitleId = g_DashList[g_FallbackDash];
	}
	...
	for (size_t i=0; i<g_GameList.size(); i++) {
        ...
        if (game.titleId == finalTitleId) {
			...
			if (g_ShowGameIcon && game.iconExists && finalTitleId != 0x00000166 && finalTitleId != 0x00000167 && finalTitleId != 0xFFFE07D1) {
				if (FetchDiscordAsset(finalTitleId, game.secondaryId, dynamicAssetBuffer, 256)) 
				{
				...
				{
					XexUtils::Log::Print("[XeCord] Error: Icon Fetch failed, using default icon. Requested TID: %08X.", titleId);
					finalGameIconExists = false;
				}
			}

			finalLargeImage = finalGameIconExists ? finalLargeImage : (titleId == 0xFFFE07D2) ? "mp:app-assets/1410522131762253927/1410522692959998023.png" : "mp:app-assets/1410522131762253927/1417550167074406711.png";
			finalSmallImage = (titleId == 0xFFFE07D2) ? (finalGameIconExists ? "mp:app-assets/1410522131762253927/1410522692959998023.png" : (g_FallbackTo360SmallForXbox1 ? "mp:app-assets/1410522131762253927/1410522692968382586.png" : "")) : (finalGameIconExists ? "mp:app-assets/1410522131762253927/1410522692968382586.png" : "");
	...
	if (finalTitleId == 0x00000166) // Aurora
	{
		...
		finalLargeImage = "mp:app-assets/1410522131762253927/1410523603815895132.png"; 
		finalSmallImage = g_ShowSmallImageOnCustomDash 
	...
	else if (finalTitleId == 0x00000167) // Freestyle 3
	{
		...
		finalLargeImage = "mp:app-assets/1410522131762253927/1410522692414738493.png"; 
		finalSmallImage = g_ShowSmallImageOnCustomDash 
							? "mp:app-assets/1410522131762253927/1410522692968382586.png" 
							: "";
	}
	else if (finalTitleId == 0xFFFE07D1) // Xbox 360 Dashboard
	{
		...
		finalLargeImage = "mp:app-assets/1410522131762253927/1417550167074406711.png";
		finalSmallImage = "";
	}

    XexUtils::Log::Print("[XeCord] Game Changed! Updating Presence to: %s (%08X).", finalGameName, finalTitleId);

	if (!g_ShowGameIcon) {
		finalLargeImage = (titleId == 0xFFFE07D2) ? (g_AlwaysShowOGXboxIcon ? "mp:app-assets/1410522131762253927/1410522692959998023.png" : "mp:app-assets/1410522131762253927/1417550167074406711.png") : "mp:app-assets/1410522131762253927/1417550167074406711.png";
		finalSmallImage = (titleId == 0xFFFE07D2) ? (g_FallbackTo360SmallForXbox1 ? "mp:app-assets/1410522131762253927/1410522692968382586.png" : "") : "";
	}
	...
	if (g_ShowConsole) {
		...
		if (g_ShowKernelVersion) {
			if (g_ShowBuildNumberOnly) {
				addinfos << " " << "[" << ((g_CustomVersionNumber == 0) ? XboxKrnlVersion->Build : g_CustomVersionNumber) << "]";
			}
			else {
				addinfos << " " << "[2.0." << ((g_CustomVersionNumber == 0) ? XboxKrnlVersion->Build : g_CustomVersionNumber) << ".0]";
			}
		}
	...
	if (g_ShowSmallImage && strlen(finalSmallImage) > 0) {
		if (g_SwapImages) {
			...
			finalLargeImage = (titleId == 0xFFFE07D2) ? "mp:app-assets/1410522131762253927/1410522692959998023.png" : "mp:app-assets/1410522131762253927/1417550167074406711.png";
		}
	...
	if (g_EpochMillisecondsStart > 0) epochmillisecondss << "\"timestamps\":{\"start\":" << g_EpochMillisecondsStart << "},";
    ...
    sprintf_s(json, BUF_SIZE, 
        "{\"op\":2,\"d\":{"
            ...
            "\"properties\":{"
                ...
                "\"client_launch_id\":\"%s\","
                "\"browser_user_agent\":\"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) discord/1.0.9216 Chrome/138.0.7204.251 Electron/37.6.0 Safari/537.36\","
...
bool SendHeartbeat(DiscordState* state)
{
    ...
    if (currentTime - state->lastHeartbeatTime >= (unsigned long)(state->heartbeatInterval))
    {
        char json[64];
...
{
    if (!fullPayload || payloadLen <= 0 || !key) return false;
    ...
    while (searchPtr < endPtr)
    {
        ...
        while (valPtr < endPtr && (*valPtr == ' ' || *valPtr == '\t' || *valPtr == '\n')) {
            valPtr++;
        }

        if (valPtr >= endPtr) return false;

        if (valPtr + 4 <= endPtr && strncmp(valPtr, "null", 4) == 0) return false; 
...
/*bool JSONStringEquals(const char* payload, int payloadLen, const char* key, const char* expectedValue)
{
...
void ProcessPacket(DiscordState* state, char* payload, int payloadLen)
{
    ...
    if (ParseJSONInt(payload, payloadLen, "\"op\":", &opCode))
    {
        switch (opCode)
        {
            ...
            case 10:
                if (ParseJSONInt(payload, payloadLen, "\"heartbeat_interval\":", &val)) {
                    state->heartbeatInterval = val;
                    XexUtils::Log::Print("[XeCord] Discord Heartbeat Interval: %d ms.", val);
                }
                break;
...
{
    if (!pluginPath) return E_POINTER;
    ...
    for (int i = 0; g_DriveMappings[i].mount != NULL; i++)
    {
        ...
        if (strncmp(pluginPath, devicePrefix, devLen) == 0)
        {
            if (pluginPath[devLen] == '\\' || pluginPath[devLen] == '\0')
            {
                ...
                if (FAILED(hr)) {
                    XexUtils::Log::Print("[XeCord] Error: Failed to mount %s.", mountPoint);
                }
...
void LoadConfig(char* iniPath) {
	...
	g_AlwaysShowOGXboxIcon = reader.GetBoolean("Presence", "AlwaysShowOGXboxIcon", true);
	g_FallbackTo360SmallForXbox1 = reader.GetBoolean("Presence", "FallbackTo360SmallForXbox1", false);
	...
	g_ShowKernelVersion = reader.GetBoolean("Presence", "ShowKernelVersion", false);
	g_ShowBuildNumberOnly = reader.GetBoolean("Presence", "ShowBuildNumberOnly", false);
	g_CustomVersionNumber = reader.GetInteger("General", "CustomVersionNumber", 0);
...
void GatewayThread(void *pArgs)
{
	...
	while (true) {
		...
		if (g_ShowNotifications && discordFirstConnect) {
			XexUtils::Xam::XNotify("XeCord - Connecting...", XexUtils::Xam::XNOTIFYUI_TYPE_FRIENDONLINE);
		}

		XexUtils::Socket* ds = new XexUtils::Socket("gateway.discord.gg", 443, true);
		state->socket = ds;
    
		ds->AddECTrustAnchor(EC_DN, sizeof(EC_DN), EC_Q, sizeof(EC_Q), XexUtils::Socket::Curve_secp384r1);
		...
		if (PerformHandshake(state)) 
		{
			...
			int timeoutMs = 100;
			setsockopt(ds->m_Socket, SOL_SOCKET, SO_RCVTIMEO, (const char*)&timeoutMs, sizeof(timeoutMs));
			...
			if (g_ShowNotifications && discordFirstConnect) {
				XexUtils::Xam::XNotify("XeCord - Connected!", XexUtils::Xam::XNOTIFYUI_TYPE_FRIENDONLINE);
			...
			while (state->isConnected)
			{
				...
				if (len >= 2) 
				{
					...
					if (payloadLen == 126) {
						...
						payloadLen = *reinterpret_cast<unsigned short*>(&uBuffer[2]);
					}
					...
					if ((size_t)payloadLen < 8192 - (size_t)headerSize)
					{
						...
						{
							char* writePtr = state->recvBuffer + headerSize + currentPayloadBytes;
							int bytesNeeded = payloadLen - currentPayloadBytes;

							int r = ds->Receive(writePtr, bytesNeeded);

							if (r <= 0) {
								XexUtils::Log::Print("[XeCord] Error: Connection died while downloading packet fragment.");
								state->isConnected = false; 
								break; 
							}
    ...
    switch (reason)
    {
		case DLL_PROCESS_ATTACH:
		{
			...
			*reinterpret_cast<uint16_t *>(patchAddress) = 0x4800;
			...
			XexUtils::ThreadEx(reinterpret_cast<PTHREAD_START_ROUTINE>(EpochMillisecondsThread), (void*)0, EXCREATETHREAD_FLAG_SYSTEM);
			XexUtils::ThreadEx(reinterpret_cast<PTHREAD_START_ROUTINE>(GatewayThread), (void*)0, EXCREATETHREAD_FLAG_SYSTEM);
